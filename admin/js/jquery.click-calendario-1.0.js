...
jQuery.fn.calendario = function(options){	
	...
	this.each(function(){
		...
		idCalendario = 'cal_' + settings["target"].replace("#","") //idChamada;
		...
		function preencher_calendario(idCalendario){
			...
			$('#'+idCalendario+' ul.lista_dia li a').click(function(){
				...
				var mes = (1 + parseInt($.trim($(this.parentNode.parentNode.parentNode).find('input[name="calendarioMes"]').val()))).toString();
				if(mes.length==1)mes = '0'+mes;
				var ano = parseInt($.trim($(this.parentNode.parentNode.parentNode).find('input[name="calendarioAno"]').val()));					
		...
		function navegacaoCalendario(idCalendario){
			...
			if(ano == anoMinimo && mes == mesMinimo){
				...
				$('#'+idCalendario+' a.bt_voltar_mes').click(function(){			
					mes = parseInt($('input[name="calendarioMes"]').val());
					ano = parseInt($('input[name="calendarioAno"]').val());
					mes--;
					if(mes < 0){
						mes = 11;
						ano--;
